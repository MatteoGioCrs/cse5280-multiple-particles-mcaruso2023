...
The **Anisotropic** model significantly **reduces deadlocks**.

* **Why?** In an Isotropic system, two particles heading toward each other create a "force standoff" where they push each other back with equal strength, causing them to jitter (oscillate) or stop entirely (deadlock).
* In the Anisotropic system, the "broken symmetry" means a particle doesn't push back against someone behind it. This allows the "pressure" from the back of the crowd to actually push the front particles forward toward the goal, rather than creating a counter-force that jams the whole group.
